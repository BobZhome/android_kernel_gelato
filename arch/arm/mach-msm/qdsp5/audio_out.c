...
//20110606 hyeongwoo.seo@lge.com add USE_RAM_BUFFER [START]
/*
    FIXME : this is workaround solution for the first audio chopping problem
    double buffer operation bug...
*/
#define  USE_RAM_BUFFER
//20110606 hyeongwoo.seo@lge.com Write description here in detail [END]
			   ...
			   size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		#ifdef USE_RAM_BUFFER
		if(audio->out_ram[audio->out_head].used==0){
			audio->out_ram[audio->out_head].used = audio->out_ram[audio->out_head].size;
		}
		memcpy(audio->out[audio->out_head].data,audio->out_ram[audio->out_head].data,audio->out_ram[audio->out_head].used);
		
		if (copy_from_user(audio->out_ram[audio->out_head].data, buf, xfer)) {
			rc = -EFAULT;
			break;
		}
		audio->out_ram[audio->out_head].used= xfer;
		#else /*QBSP Original Code*/
...
#ifdef USE_RAM_BUFFER
	if(!audio->out_ram[0].data){
		char *pchar;
		audio->out_ram[0].data= kmalloc(DMASZ, GFP_KERNEL);
		if(!audio->out_ram[0].data){
			MM_ERR("could not allocate tmp buffers\n");
			rc = -ENOMEM;
			goto done;		
		}
		pchar = (char *)&audio->out_ram[0].data;
		audio->out_ram[1].data = pchar + BUFSZ;		
		audio->out_ram[0].size = BUFSZ;		
		audio->out_ram[1].size = BUFSZ;
	}
	audio->out_ram[0].used = 0;
	audio->out_ram[1].used = 0;
	memset(audio->out_ram[0].data,0x00,DMASZ);
#endif /*USE_RAM_BUFFER*/
