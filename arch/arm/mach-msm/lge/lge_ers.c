		...
		unsigned long event, void *ptr)
{
	...
	if (fd < 0) {
		printk("[%s] tombstone 01 \n", __func__);
		return NOTIFY_DONE;
	}

	printk("[%s] report_start(%d), start(%d) \n", __func__, report_start, start);

	if (report_start < start) {
		printk("[%s] tombstone 02 \n", __func__);
		sys_write(fd, &data[report_start], start - report_start);
	} else {
		printk("[%s] tombstone 03 \n", __func__);
		sys_write(fd, &data[report_start], size - report_start);
		sys_write(fd, &data[0], start);
	}
	...
	printk("[%s] tombstone 04 \n", __func__);
	
	set_fs(oldfs);

	printk("[%s] tombstone 05 \n", __func__);
