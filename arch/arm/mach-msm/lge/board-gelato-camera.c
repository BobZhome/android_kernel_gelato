...
static uint32_t camera_off_gpio_table[] = {
	...
	GPIO_CFG(GPIO_CAM_MCLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
		GPIO_CFG_2MA), /* MCLK */
};
...
static uint32_t camera_on_gpio_table[] = {
	...
	GPIO_CFG(GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
		GPIO_CFG_4MA), /* MCLK */
};
...
{
	printk(KERN_ALERT "[smiledice] >> %s START (HYNIX SENSOR)\n", __func__);
	...
	vreg_gp7 = vreg_get(0, "gp7");
	vreg_set_level(vreg_gp7, 1800);
	...
	rc = aat28xx_ldo_set_level(dev, LDO_CAM_IOVDD_NO, 1800);
	if (rc < 0) {
	...
	power_off_fail:
		...
		printk(KERN_ALERT "[smiledice] << %s END \n", __func__);
		return rc;
	...
	int camera_power_on(void)
	{
		printk(KERN_ALERT "[smiledice] >> %s START (SAMSUNG SENSOR)\n", __func__);
...
// LGE_CHANGE_S 2011.01.26 [jongkwon.chae@lge.com] [gelato] sensor porting
	...
	power_off_fail:
		...
		printk(KERN_ALERT "[smiledice] << %s END (rc: %d) \n", __func__, rc);
		return rc;
	}
