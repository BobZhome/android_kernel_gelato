...
extern int lge_get_hw_version(void);

int lge_get_board_pcb_version(void)
{
	static int pcb_version = HW_PCB_UNKNOWN;

	if(pcb_version == HW_PCB_UNKNOWN) {
		pcb_version = lge_get_hw_version();
	}

	return pcb_version;
}

void lge_set_hw_version_string(char *pcb_version)
{
	enum hw_pcb_version_type hw_version;

	hw_version=lge_get_board_pcb_version();
...
EXPORT_SYMBOL(lge_set_hw_version_string);
