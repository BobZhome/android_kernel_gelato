...
/* LGE_CHANGES_S [moses.son@lge.com] 2011-04-08, to avoid rpc write when smd rpc channel is closed */
#if defined(CONFIG_MACH_MSM7X27_GELATO)
#include "smd_private.h"
extern uint32_t smsm_get_state(uint32_t smsm_entry);
#endif
/* LGE_CHANGES_E [moses.son@lge.com] 2011-04-08 */
...
/* LGE_CHANGES_S [moses.son@lge.com] 2011-04-08, to avoid rpc write when smd rpc channel is closed */
#if defined(CONFIG_MACH_MSM7X27_GELATO)
	if(!(smsm_get_state(SMSM_APPS_STATE)&SMSM_RPCINIT))
	{
		printk(KERN_ERR "%s smd rpc is closed\n", __func__);
		return -EINVAL;
	}
#endif	
/* LGE_CHANGES_E [moses.son@lge.com] 2011-04-08 */
