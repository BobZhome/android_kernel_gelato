...
#define TEMPERATURE_BLOCK_ATTR(_name)				\
{									\
	.attr = { .name = #_name, .mode = 0664 },	\
	.show = temperature_block_show_property,			\
	.store = temperature_block_store_property,			\
}
#endif
...
static ssize_t temperature_block_show_property(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	ssize_t ret;
	...
	if (off == POWER_SUPPLY_PROP_BLOCK_TEMPERTATURE)
		return sprintf(buf, "%d", value.intval);

	return 0;
}

extern void temperature_block_set(int no_stop);

static ssize_t temperature_block_store_property(struct device *dev,
		  struct device_attribute *attr,
		  const char *buf, size_t count)
{
	int ret = -EINVAL;
 ...
 */
	...
	TEMPERATURE_BLOCK_ATTR(temp_block),	
#endif
};
