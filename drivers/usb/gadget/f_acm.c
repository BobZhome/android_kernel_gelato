...
/*	LGE_BRYCE_S		yk.kim@lge.com		10.08.10
	prevent notify_request zero
*/
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
	if (acm->notify_req)
#endif
	gs_free_req(acm->notify, acm->notify_req);
/*	LGE_BRYCE_E		yk.kim@lge.com	*/
	kfree(acm);
}
...
#if 0	/* LGE_CHANGES_S [moses.son@lge.com] 2011-02-09, bryce porting  */
...
/* ADD:0010634 Bryce USB composition */
#ifdef CONFIG_LGE_USB_GADGET_FUNC_BIND_ONLY_INIT
int acm_modem_bind_config(struct usb_configuration *c)
{

	int ret =   gserial_setup(c->cdev->gadget, 2); //0: for acm , 1: for nmea
	if (ret == 0)
...
static struct android_usb_function acm_modem_function = {
	.name = "acm",
	.bind_config = acm_modem_bind_config,
};

static int __init init(void)
{
	printk(KERN_INFO "f_acm.c init\n");
	android_register_function(&acm_modem_function);
	return 0;
}
module_init(init);
#endif
/* END: [yk.kim@lge.com] 2010-11-08 */
#endif	/* LGE_CHANGES_E [moses.son@lge.com] 2011-02-09 */
