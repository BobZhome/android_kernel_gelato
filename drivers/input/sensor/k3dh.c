...
#define PM_OFF          	0x00
#define PM_NORMAL       	0x60  // 200Hz 5ms
...
static unsigned char accel_cal = 0;
...
static int k3dh_hw_init(struct k3dh_data *kr)
{
	...
	buf[4] = kr->resume_state[3] = kr->resume_state[3] | CTRL_REG4_HR;
...
{
	int err;
	u8 ctrl1[2] = {CTRL_REG1, PM_OFF};
    u8 ctrl4[2] = {CTRL_REG4, (kr->resume_state[3] & ~CTRL_REG4_HR)};

	printk("%s, K3DH Power Off\n");
		
	err = k3dh_i2c_write(kr, ctrl1, 1);
	err = k3dh_i2c_write(kr, ctrl4, 1);
	if (err < 0)
...
int k3dh_update_g_range(struct k3dh_data *kr, u8 new_g_range)
{
	...
	if (atomic_read(&kr->enabled)) {
		 ...
		 *  (marked by X) */
		buf[0] = CTRL_REG4;
		buf[1] = new_g_range | CTRL_REG4_HR;
		err = k3dh_i2c_write(kr, buf, 1);
		if (err < 0)
			return err;
	}

	kr->resume_state[3] = new_g_range | CTRL_REG4_HR;
	kr->shift_adj = shift;
...
int k3dh_update_odr(struct k3dh_data *kr, int poll_interval)
{
	...
	printk(KERN_INFO "%s, polling Hz : %d\n",__func__, config[1]);
...
//	err = k3dh_i2c_read(kr,&config,1);
//	dev_info(&kr->client->dev, "(x, y ,z) = (%d, %d, %d), config: %d \n", xyz[0],xyz[1], xyz[2], config);
...
static ssize_t k3dh_cal_save(struct device *dev, struct device_attribute *attr, 
	const char *buf, size_t count)
{
	int mode = 0;
	sscanf(buf, "%d", &accel_cal);
	return count;
}

static ssize_t k3dh_cal_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	return sprintf(buf, "%d\n", accel_cal);
}
...
static DEVICE_ATTR(cal, S_IRUGO, k3dh_cal_show, k3dh_cal_save);
			   ...
			   const struct i2c_device_id *id)
{
	...
	kr->resume_state[1] = 0;
	kr->resume_state[2] = 0;
	kr->resume_state[3] = 0 | CTRL_REG4_HR;
	kr->resume_state[4] = 0;
