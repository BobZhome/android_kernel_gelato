		...
		struct device_attribute *attr, char *buf)
{
	int mode=0;
   u8 regaddr;
   u8 ctrl1, ctrl2, ctrl3, fine_x, fine_y, fine_z;
   unsigned char ctrl4[2];
   ...
   i2c_master_send(ami306_i2c_client, &regaddr, 1);
   i2c_master_recv(ami306_i2c_client, ctrl4, 2);             
   
   regaddr = 0x6c; //OFFX_REG;
   i2c_master_send(ami306_i2c_client, &regaddr, 1);
   i2c_master_recv(ami306_i2c_client, &fine_x, 1);

   regaddr = 0x72; //OFFY_REG;
   i2c_master_send(ami306_i2c_client, &regaddr, 1);
   i2c_master_recv(ami306_i2c_client, &fine_y, 1);

   regaddr = 0x78; //OFFZ_REG;
   i2c_master_send(ami306_i2c_client, &regaddr, 1);
   i2c_master_recv(ami306_i2c_client, &fine_z, 1);

   return sprintf(buf, "%d %x %x %x %x %x %x %x %x\n", 
   	mode, ctrl1, ctrl2, ctrl3, ctrl4[0], ctrl4[1], fine_x, fine_y, fine_z);
}
