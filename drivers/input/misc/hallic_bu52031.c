...
static struct bu52031_platform_data *bu52031_pdata = NULL;
...
static void bu52031_timer(unsigned long arg)
{
	...
	current_state = gpio_get_value(bu52031_pdata->irq_pin);
	...
	if (current_state == last_state) {
		...
		mod_timer(&dev->timer, jiffies + (bu52031_pdata->prohibit_time * HZ / 1000));
	}
...
static int bu52031_irq_handler(int irq, void *dev_id)
{
	...
	last_state = gpio_get_value(bu52031_pdata->irq_pin);
...
{
	if (gpio_get_value(bu52031_pdata->irq_pin) == 1) {
		hall_ic_on = 0;
	}
	else {
		hall_ic_on = 1;
								 ...
								 const char *buf, size_t size)
{
	...
	bu52031_pdata->prohibit_time = value;
...
static int bu52031_probe(struct platform_device *pdev)
{
	...
	bu52031_pdata = pdev->dev.platform_data;

	printk(KERN_INFO"%s : irq = %d, prohibit_time = %d\n",__func__,bu52031_pdata->irq_pin,bu52031_pdata->prohibit_time);

	if (!bu52031_pdata->irq_pin) {
	...
	setup_timer(&dev->timer, bu52031_timer, (unsigned long)dev);
	
	hall_ic_enable = 1; 
	dev->irq_num = gpio_to_irq(bu52031_pdata->irq_pin);

	printk(KERN_INFO"%s : dev->irq_num = %d, hallic_dev->irq_num = %d\n",__func__,dev->irq_num,hallic_dev->irq_num);
	
	ret = request_irq(hallic_dev->irq_num, bu52031_irq_handler,
...
static int bu52031_remove(struct platform_device *pdev) 
{
	struct hall_ic_device *dev = (struct hall_ic_device *)hallic_dev;
...
static struct platform_driver bu52031_driver = {
	.probe		= bu52031_probe,
	.remove		= bu52031_remove,
	.driver		= {
		.name		= "hall-ic",
		.owner		= THIS_MODULE,
	},
...
static int __init bu52031_init(void)
{
	return platform_driver_register(&bu52031_driver);
}

static void __exit bu52031_exit(void)
{
	platform_driver_unregister(&bu52031_driver);
}

module_init(bu52031_init);
module_exit(bu52031_exit);

MODULE_DESCRIPTION("bu52031 hall ic driver for Android");
MODULE_LICENSE("GPL");
