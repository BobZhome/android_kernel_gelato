...
/* LGE_CHANGE_S, [dongseok.ok@lge.com], 2011-02-10, <Add BCM4330> */ 
#ifdef CONFIG_BCM4330_GPIO_WL_RESET
#define WLAN_RESET_GPIO CONFIG_BCM4330_GPIO_WL_RESET
#endif
/* LGE_CHANGE_E, [dongseok.ok@lge.com], 2011-02-10, <Add BCM4330> */

/* LGE_CHANGE_S [dongseok.ok@lge.com] 2011-02-10, for gpio_to_irq */
#if defined(CONFIG_LGE_BCM432X_PATCH)
#include <asm/gpio.h>
/*LGE_CHANGE_S, [dongseok.ok@lge.com], 2011-02-10, mmc_fmax is 24576000Hz for Wi-Fi */ 
#define BRCM_WLAN_SLOT 2
/*LGE_CHANGE_E, [dongseok.ok@lge.com], 2011-02-10, mmc_fmax is 24576000Hz for Wi-Fi */ 
#endif
/* LGE_CHANGE_E [dongseok.ok@lge.com] 2011-02-10, for gpio_to_irq */

#define DRIVER_NAME "msm-sdcc"
/* LGE_CHANGE_S [dongseok.ok@lge.com] 2011-02-10, Delay time for work-queue to rescan */
#define TIME_STEP ( 4 * HZ / 5 )
/* LGE_CHANGE_E [dongseok.ok@lge.com] 2011-02-10, Delay time for work-queue to rescan */
...
/* LGE_CHANGE_S [dongseok.ok@lge.com] 2011-02-10, Support Host Wakeup */
...
#endif
/* LGE_CHANGE_E [dongseok.ok@lge.com] 2011-02-10, Support Host Wakeup */
...
/*LGE_CHANGE_S, [dongseok.ok@lge.com], 2011-02-10, when PIO_irq, delay is given */ 
#if defined(CONFIG_LGE_BCM432X_PATCH)
		if (host->mmc->card != NULL && host->mmc->card->type == MMC_TYPE_SDIO){ 
			msmsdcc_delay(host);
		}
#endif	
/*LGE_CHANGE_E, [dongseok.ok@lge.com], 2011-02-10, when PIO_irq, delay is given */ 
		/* Unmap the buffer */
...
msmsdcc_check_status(unsigned long data)
{
	...
	printk("<choi> msmsdcc_check_status start \n");
	...
	if (!host->plat->status) {
		...
		if (status ^ host->oldstat) {
			printk("<choi> msmsdcc_check_status status changed %d\n",status);
...
/* LGE_CHANGE_E, [jongpil.yoon@lge.com], 2011-01-24, <resolve the 'bus down' issue> */
	...
	printk("<choi> msmsdcc_check_status end \n");
}
...
static void msmsdcc_early_suspend(struct early_suspend *h)
{
	...
	unsigned long flags;
/* LGE_CHANGE_S [dongseok.ok@lge.com] 2011-02-10, don't do this to WLAN */
...
/* LGE_CHANGE_S [dongseok.ok@lge.com] 2011-02-10, don't do this to WLAN */
...
#endif
	{
		...
		if (!host->plat->status) {
			...
			host->eject = !status;
			if (status ^ host->oldstat) {
				 printk("<choi> %s: Slot status change detected (%d -> %d)\n",
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if (host->plat->status_irq != gpio_to_irq(WLAN_RESET_GPIO)) {
			pm_runtime_get_noresume(dev);
			if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO)//added by HappyMill for sdcard detection
...
/* LGE_CHANGE_S, [dongseok.ok@lge.com], 2011-02-10, <do not do for wifi> */
...
#endif			
/* LGE_CHANGE_E, [dongseok.ok@lge.com], 2011-02-10, <do not do for wifi> */			
...
/* LGE_CHANGE_S, [dongseok.ok@lge.com], 2011-02-10, <never sleep policy - host wakeup> */
#if defined(CONFIG_BRCM_LGE_WL_HOSTWAKEUP)
		//if ( mmc->card && mmc->card->type == MMC_TYPE_SDIO) {
		if (host->plat->status_irq == gpio_to_irq(WLAN_RESET_GPIO)) {
...
#endif	
/* LGE_CHANGE_E, [dongseok.ok@lge.com], 2011-02-10, <never sleep policy - host wakeup> */
