...
#if 1//def CONFIG_LGE_APPS_FACTORY_RESET
	int skipCheck = 0;
#endif
...
#if 1//def CONFIG_LGE_APPS_FACTORY_RESET
	if(YAFFS_CHECKPOINT_VERSION > 3)
	{
		// when skipErasedCheck is 0, chunk erased status will be checked and the chunk will be erased if not.
		// if this is the persist partition that the factory reset flag is saved, skip checking erased status
		if((dev)&&(!strncmp(dev->param.name, "persist", 7)))
			skipCheck = 1;
	}
#endif
	...
	do {
		...
		if (!bi->skipErasedCheck) {
			...
			if (erasedOk != YAFFS_OK) {
				...
				if(skipCheck == 1)
				{
					printk(KERN_INFO "%s, %s : skipErasedCheck\n", __func__, dev->param.name);
					continue;
				}
#endif
			        ...
			        int chunkInNAND, int inScan)
{
	...
	tn = yaffs_AddOrFindLevel0Tnode(dev,
					...
					chunkInInode,
					NULL);
#if 1//def LGE_YAFFS2_SKIP_CHUNK_FAIL_IN_SUPER
	if (!tn)
	{
		extern int isReadSuper;
		if(isReadSuper)
		{
			yaffs_DeleteChunk(dev, chunkInNAND, 1, __LINE__);
			printk(KERN_INFO "yaffs: %s : yaffs_AddOrFindLevel0Tnode failed\n",__func__);
			return YAFFS_OK;
		}
					...
					yaffs_Object *obj)
{
	...
	if(obj == NULL)
	{
		printk(KERN_ERR"%s : <choi> obj is null\n",__func__);
		return;
	}
